...
function path2url($file, $vname_url='' , $Protocol='https://') {
	return str_replace( "phpsane", "$vname_url" , $Protocol.$_SERVER['HTTP_HOST'].str_replace($_SERVER['DOCUMENT_ROOT'], '/', $file));
}
// ---------------------------------------------------------------------
...
function get_scanner_source_supported($scanner_name) {
	$source_info = explode('|', get_config_option($scanner_name, "source"));
	return $source_info[0];
}
function get_scanner_source_options($scanner_name) {
	return get_config_option_array($scanner_name, "source",1);
}
function get_scanner_source_default($scanner_name) {
	$source_info = explode('|', get_config_option($scanner_name, "source"));
...
function save_scanner_config($scanner_name, 
	...
	file_put_contents($file_path, "source:".($source_supported ? 'true' : 'false')."|".implode(';', $source_list)."|{$source_default}\r\n", FILE_APPEND);
}
